        ...
        if(tuplas!=0) 
		{
            ...
            for (rowi = 0; rowi<tuplas; rowi++) 
            {
			  ...
			  for(i=0;i<tt->canColumns;++i)
    		  {
				...
				// verificar la gestion de valores nulos para cadena y fecha
				setValue(obj,i,dato);
				//free(dato);
		 	  }
...
void destroyObj(void *objFree)
{
   ...
   for(i=0;i<tt->canColumns;++i)
   {
    ...
    free(obj->col_value);
	//free(obj->ds);// no liberar por que es un array del pseudo objeto, no se puede liberar esa memoria. Column cols_XXX..
	free(obj);
}
//----------------------------------------------------
